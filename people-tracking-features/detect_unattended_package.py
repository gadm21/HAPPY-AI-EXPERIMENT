...
def drawTrackedBag(imgDisplay):
    for fid in bagtracker.faceTrackers.keys():
        ...
        cv2.putText(
            imgDisplay, text, textLoc, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2
        )
...
if __name__ == "__main__":
    ...
    threshold = 0.85
    bagthreshold = 0.4
    cap = cv2.VideoCapture("videos/RSA loss and found/A0051.mov")
    flag, frame = cap.read()
    assert flag == True
    ...
    out = cv2.VideoWriter(
        "output_videos/rsa_food_court.avi", fourcc, fps, (width, height)
    )
        ...
        if frame_count % frame_interval == 0:
            ...
            for i in range(len(boxes)):
                ...
                if classes[i] == 1 and scores[i] >= threshold:
                    ...
                    classes[i] == 27 or classes[i] == 31 or classes[i] == 33
                ) and scores[i] > bagthreshold:
                    box = boxes[i]
                    matchedID = bagtracker.getMatchId(
                        img, (box[1], box[0], box[3], box[2])
                    )
                    if matchedID is None:
                        bag += 1
                        bagtracker.createTrack(
                            img, (box[1], box[0], box[3], box[2]), str(bag), scores[i]
                        )
