...
if __name__ == "__main__":
    ...
    while True:
        ...
        if frame_count % frame_interval == 0:
            ...
            for i in range(len(boxes)):
                ...
                if classes[i] == 1 and scores[i] >= threshold:
                    ...
                    box = boxes[i]
                    matchedID = seattracker.getMatchId(
                        img, (box[1], box[0], box[3], box[2])
                    )
                    if matchedID is None:
                        seat += 1
                        seattracker.createTrack(
                            img, (box[1], box[0], box[3], box[2]), str(seat), scores[i]
                        )
        ...
        cv2.putText(
            img,
            "Occupied Seat: {}".format(str(occupied)),
            (0, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (255, 0, 0),
        ...
        cv2.putText(
            img,
            "Free Seat: {}".format(str(unoccupied)),
            (0, 75),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 0, 255),
