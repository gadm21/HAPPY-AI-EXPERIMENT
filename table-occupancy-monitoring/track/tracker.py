...
class Tracker:
	...
	def getID(self, imgDisplay, boundingBox, tableID):
		...
		for fid in self.faceTrackers.keys():
			...
			t_width = int(tracked_position.width())
			t_height = int(tracked_position.height())
			...
			if self.table == int(tableID):
				#print("table matched")
				if ((t_x1 - t_width <= x_mid <= t_x2 + t_width) and
				(t_y1 - t_height <= y_mid <= t_y2 + t_height) and
				(x1 <= t_x_mid <= x2) and
				(y1 <= t_y_mid <= y2)):
	...
	def deleteTrack(self, imgDisplay):
		...
		for fid in self.faceTrackers.keys():
			...
			if trackingQuality < self.trackingQuality:
				...
				print("quality is " + str(trackingQuality))
				continue
