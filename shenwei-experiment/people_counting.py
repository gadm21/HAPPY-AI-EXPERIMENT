	...
	while True:
		...
		if r:
				...
				for table in tables: 
					...
					for i in range(len(boxes)):
						...
						if classes[i] == 1 and scores[i] >= threshold:
							matchedID = tracker[table_num].getMatchId(img, (x1, y1, x2, y2))
							if matchedID is None:
								id += 1
								tracker[table_num].createTrack(img, (x1, y1, x2, y2), str(id), scores[i])
			...
			for n in range(cam0['num_of_tables']):
				number += int(len(tracker[n].faceTrackers))
