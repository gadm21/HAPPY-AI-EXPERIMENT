...
class Tracker:
    ...
    def createTrack(self, imgDisplay, boundingBox, currentFaceID, score):
        ...
        print('Creating new person tracker' + str(currentFaceID))
        ...
        self.isLoitering[currentFaceID] = 1
    ...
    def getLoiteringTime(self, fid):
        return self.isLoitering[fid] / float(self.fps)
    ...
    def deleteTrack(self, imgDisplay):
        for fid in self.faceTrackers.keys():
            ...
            if relativeOutScreen > self.outOfScreenThreshold:
                print("person tracker {} Out of Screen".format(fid))
                self.fidsToDelete.append(fid)
                ...
                if (((t_x <= x_bar <= (t_x + t_w)) and
                     (t_y <= y_bar <= (t_y + t_h))) and
                        ((x <= t_x_bar <= (x + w)) and
                         (y <= t_y_bar <= (y + h)))):
                    self.fidsToDelete.append(fid)
                    print('delete overlap person tracker {}, {}'.format(id, fid))
                    break
