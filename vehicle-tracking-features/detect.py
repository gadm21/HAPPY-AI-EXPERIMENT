...
def drawTrackedObject(imgDisplay):
	for oid in tracker.objectTrackers.keys():
		...
		cv2.rectangle(imgDisplay, (t_x, t_y-opt.getTrueOption()),
					  (t_x+80,t_y),rectColor, -1)

		counter = 0
		if opt.type:
			cv2.putText(imgDisplay, text, (t_x,t_y-counter),
						cv2.FONT_HERSHEY_SIMPLEX,
						0.4, (255, 255, 255), 1)
			counter += 10

		if opt.color:
			cv2.putText(imgDisplay, textColor, (t_x,t_y-counter),
						cv2.FONT_HERSHEY_SIMPLEX,
						0.4, (255, 255, 255), 1)
			counter += 10

		if opt.speed:
			cv2.putText(imgDisplay, textSpeed, (t_x,t_y-counter),
						cv2.FONT_HERSHEY_SIMPLEX,
						0.4, (255, 255, 255), 1)
			counter += 10

		if opt.stay:
			cv2.putText(imgDisplay, textStay, (t_x,t_y-counter),
						cv2.FONT_HERSHEY_SIMPLEX,
						0.4, (255, 255, 255), 1)
			counter += 10
	...
	while True:  # fps._numFrames < 120
		...
		if (frame_count%frame_interval)==0:
			...
			for i in range(boxes.shape[0]):
				if scores[i] > 0.5:
					box = boxes[i]
					(ymin, xmin, ymax, xmax) = (int(box[0]*im_height),int(box[1]*im_width),
					...
					matchedID = tracker.getMatchId(frame.copy(),(xmin,ymin,xmax,ymax))
					...
					if matchedID is None:
						...
						tracker.createTrack(frame.copy(),(xmin,ymin,xmax,ymax),currentTrackID)
		...
		if opt.toll:
			...
			cv2.rectangle(frame, (b1x1,b1y1),
						  (b1x2,b1y2),
						  (255,0,0),2)

			cv2.rectangle(frame, (b2x1,b2y1),
						  (b2x2,b2y2),
						  (255,0,0),2)
			cv2.rectangle(frame, (b3x1,b3y1),
						  (b3x2,b3y2),
						  (255,0,0),2)
			...
			cv2.putText(frame, payment, (10,100),
						cv2.FONT_HERSHEY_SIMPLEX,
						1, (255, 0, 255), 2)

			traffic = 'Vehicle: '+ str(ccount)
			cv2.putText(frame, traffic, (10,150),
						cv2.FONT_HERSHEY_SIMPLEX,
						1, (255, 0, 255), 2)
