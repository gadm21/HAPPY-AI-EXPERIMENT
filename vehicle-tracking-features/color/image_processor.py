...
class ColorWheel(object):
    ...
    def get_dominant_colors(self):
        ...
        return [x for x in self.rgb if x.value >= max_color * 0.9]
...
def process_image(image):
    ...
    width_margin = int(width - (width * 0.65))
    height_margin = int(height - (height * 0.65))

    # width_margin = int(width - (width * 1))
    # height_margin = int(height - (height * 1))
    ...
    total_assessed_pixels = sum(
        [v for k, v in image_color_quantities.items() if v > 10]
    )

    # strongest_color_wheels = [(ColorWheel(k), v / float(total_pixels) * 100, ) for k, v in test.items() if v > 30]
    strongest_color_wheels = [
        (ColorWheel(k), v / float(total_assessed_pixels) * 100)
        for k, v in image_color_quantities.items()
        if v > 10
    ]
    ...
    for color, strength in final_colors.items():
        ...
        if color.__name__ == "Gray":
            gray = strength
            continue
            # print("%s - %s" % (color.__name__, strength, ))
        if maxStrength < strength:
            maxStrength = strength
            result = color.__name__

            # if maxStrength < 15 and maxStrength > 5:
    if maxStrength < 15:
        result = "Gray"
        # elif maxStrength <= 5:
        # 	result = 'None'
        # print(result,maxStrength)
    return result
    # image.show()


def toll_process_image(image):
    image_color_quantities = {}
...
if __name__ == "__main__":
    ...
    t1 = time.time()
    process_image(image)
    print(str(time.time() - t1) + " elapsed")
